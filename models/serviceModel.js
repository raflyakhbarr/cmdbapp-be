...
// Lazy load CMDB model to avoid circular dependency
let cmdbModel = null;
const getCMDBModel = () => {
  if (!cmdbModel) {
    cmdbModel = require('./cmdbModel');
  }
  return cmdbModel;
};
...
const updateServiceStatus = async (id, status) => {
  ...
  if (workspaceId && (status === 'inactive' || status === 'disabled' || status === 'maintenance')) {
    ...
    if (status === 'inactive' || status === 'maintenance' || status === 'decommissioned' || status === 'disabled') {
      ...
      console.log(`📦 ============================================`);
      console.log(`📦 Source: Service ID ${id}`);
      ...
      try {
        // Get all service items in this service
        const serviceItemsForCmdb = await pool.query(
          `SELECT id, name FROM service_items WHERE service_id = $1 AND workspace_id = $2`,
          [id, workspaceId]
        );

        console.log(`📦 Processing CMDB item propagation for ${serviceItemsForCmdb.rows.length} service items`);

        for (const serviceItem of serviceItemsForCmdb.rows) {
          console.log(`\n📦 Processing service item "${serviceItem.name}" (ID: ${serviceItem.id})`);

          // Get all connections where this service item is the source to CMDB items
          const cmdbConnectionsResult = await pool.query(
            `SELECT c.*, ct.propagation, ct.type_slug as connection_type_name
             FROM connections c
             LEFT JOIN connection_type_definitions ct ON ct.type_slug = c.connection_type
             WHERE c.source_service_item_id = $1
               AND c.workspace_id = $2
               AND c.target_id IS NOT NULL`,
            [serviceItem.id, workspaceId]
          );

          console.log(`📦 Found ${cmdbConnectionsResult.rows.length} CMDB item connections from this service item`);

          for (const conn of cmdbConnectionsResult.rows) {
            const targetItemId = conn.target_id;
            const propagation = conn.propagation || 'source_to_target';

            console.log(`📦 Connection to CMDB Item ${targetItemId}: type=${conn.connection_type_name}, propagation=${propagation}`);

            if (propagation === 'source_to_target' || propagation === 'both') {
              const targetItemResult = await pool.query(
                'SELECT id, name, status FROM cmdb_items WHERE id = $1',
                [targetItemId]
              );

              if (targetItemResult.rows.length > 0) {
                const targetItem = targetItemResult.rows[0];
                console.log(`📦 Target CMDB Item: "${targetItem.name}" (Current Status: ${targetItem.status})`);

                if (targetItem.status === 'active') {
                  const cmdbModel = getCMDBModel();
                  await cmdbModel.updateItemStatus(targetItemId, status);
                  console.log(`✅ Updated CMDB Item "${targetItem.name}" status to ${status}`);
                } else {
                  console.log(`⏸️ Skipped CMDB Item "${targetItem.name}" - already has status ${targetItem.status}`);
                }
              }
            } else if (propagation === 'target_to_source') {
              console.log(`⏸️ Skipped - propagation is target_to_source (target affects source, not relevant here)`);
            }
          }
        }

        console.log(`\n📦 --------------------------------------------`);
        console.log(`📦 CMDB ITEM PROPAGATION FROM SERVICE ITEMS COMPLETED`);
        console.log(`📦 ============================================\n`);

        // Emit CMDB update to refresh frontend after service item CMDB propagation
        const { emitCmdbUpdate } = getSocketFunctions();
        await emitCmdbUpdate();
        console.log(`✅ Emitted cmdb_update after service item CMDB propagation`);
      } catch (error) {
      ...
      try {
        ...
        if (connectionsResult.rows.length > 0) {
          console.log(`\n📦 ============================================`);
          console.log(`📦 CMDB ITEM PROPAGATION FROM SERVICE (DIRECT) STARTED`);
          console.log(`📦 ============================================`);
          console.log(`📦 Found ${connectionsResult.rows.length} direct service-to-CMDB connections`);

          for (const conn of connectionsResult.rows) {
            const targetItemId = conn.target_id;
            const propagation = conn.propagation || 'source_to_target';

            console.log(`\n📦 Processing direct service-to-CMDB connection to ${targetItemId}`);
            console.log(`📦 Connection type: ${conn.connection_type}, propagation: ${propagation}`);

            if (propagation === 'source_to_target' || propagation === 'both') {
              // Get target CMDB item details
              const targetItemResult = await pool.query(
                'SELECT id, name, status FROM cmdb_items WHERE id = $1',
                [targetItemId]
              );
              ...
              if (targetItemResult.rows.length > 0) {
                const targetItem = targetItemResult.rows[0];
                console.log(`📦 Target Item: "${targetItem.name}" (ID: ${targetItemId}, Current Status: ${targetItem.status})`);

                // Only update if target is currently active (don't override worse status)
                if (targetItem.status === 'active') {
                  // Update target CMDB item status
                  const cmdbModel = getCMDBModel();
                  await cmdbModel.updateItemStatus(targetItemId, status);
                  console.log(`✅ Updated CMDB Item "${targetItem.name}" status to ${status}`);
                } else {
                  console.log(`⏸️ Skipped CMDB Item "${targetItem.name}" - already has status ${targetItem.status} (worse or equal)`);
                }
              }
            } else if (propagation === 'target_to_source') {
              console.log(`⏸️ Skipped - propagation is target_to_source (target affects source, not relevant here)`);
          ...
          console.log(`\n📦 --------------------------------------------`);
          console.log(`📦 CMDB ITEM PROPAGATION FROM SERVICE COMPLETED`);
          console.log(`📦 ============================================\n`);

          // Emit CMDB update to refresh frontend after propagation
          const { emitCmdbUpdate } = getSocketFunctions();
          await emitCmdbUpdate();
          console.log(`✅ Emitted cmdb_update after service status propagation`);
        }
      } catch (error) {
        console.error(`❌ Error in CMDB item propagation from service ${id}:`, error);
...
const getAllServiceItems = (serviceId, workspaceId) => {
  return pool.query(
    `SELECT si.*, s.cmdb_item_id as cmdb_item_id
     FROM service_items si
     JOIN services s ON si.service_id = s.id
     WHERE si.service_id = $1 AND si.workspace_id = $2
...
const updateServiceItemStatus = async (id, status) => {
  ...
  if (status === 'inactive' || status === 'maintenance' || status === 'decommissioned' || status === 'disabled') {
    ...
    try {
      ...
      for (const [affectedServiceId, affectedItemIds] of Object.entries(affectedServiceItemsGrouped)) {
        try {
          ...
          // IMPORTANT: Always trigger internal propagation and CMDB item propagation
          // regardless of whether the service status was updated or not
          try {
            console.log(`\n🔄 Triggering internal propagation for service ${affectedServiceId}...`);
            ...
            const internalPropagationResult = await pool.query(
              `UPDATE service_items
               ...
               WHERE service_id = $2 AND workspace_id = $3 AND status = 'active'
               RETURNING *`,
              [status, affectedServiceId, workspace_id]
            );
            ...
            for (const internalItem of internalPropagationResult.rows) {
              await emitServiceItemStatusUpdate(internalItem.id, status, workspace_id, affectedServiceId);
              console.log(`✅ Emitted internal service item status update: item=${internalItem.id}, status=${status}, service=${affectedServiceId}`);
            }

            // IMPORTANT: Also trigger CMDB item propagation for internally updated service items
            console.log(`\n🔄 Triggering CMDB item propagation for ${internalPropagationResult.rows.length} internally updated service items`);

            for (const internalItem of internalPropagationResult.rows) {
              console.log(`\n🔄 Processing CMDB item propagation for service item ${internalItem.id} ("${internalItem.name}")`);

              // Get all connections where this service item is the source
              const cmdbConnectionsResult = await pool.query(
                `SELECT c.*, ct.propagation, ct.type_slug as connection_type_name
                 FROM connections c
                 LEFT JOIN connection_type_definitions ct ON ct.type_slug = c.connection_type
                 WHERE c.source_service_item_id = $1
                   AND c.workspace_id = $2
                   AND c.target_id IS NOT NULL`,
                [internalItem.id, workspace_id]
              );

              console.log(`🔄 Found ${cmdbConnectionsResult.rows.length} CMDB item connections for service item ${internalItem.id}`);
            ...
            console.error(`❌ Error in internal/CMDB propagation for service ${affectedServiceId}:`, error);
          }

          // IMPORTANT: Also process CMDB item propagation for AFFECTED service items
          // (the ones that were updated by cross-service propagation)
          console.log(`\n🔄 Processing CMDB item propagation for affected service items in service ${affectedServiceId}...`);

          for (const affectedItemId of affectedItemIds) {
            console.log(`\n🔄 Processing CMDB item propagation for affected service item ${affectedItemId}`);
            ...
            const cmdbConnectionsResult = await pool.query(
              `SELECT c.*, ct.propagation, ct.type_slug as connection_type_name
               ...
               WHERE c.source_service_item_id = $1
                 AND c.workspace_id = $2
                 AND c.target_id IS NOT NULL`,
              [affectedItemId, workspace_id]
            );

            console.log(`🔄 Found ${cmdbConnectionsResult.rows.length} CMDB item connections for service item ${affectedItemId}`);
      ...
      // Emit CMDB update after processing all affected services' CMDB propagations
      const { emitCmdbUpdate } = getSocketFunctions();
      await emitCmdbUpdate();
      console.log(`✅ Emitted cmdb_update after affected service items CMDB propagation`);
    } catch (error) {
    ...
    if (status === 'inactive' || status === 'maintenance' || status === 'decommissioned' || status === 'disabled') {
      ...
      console.log(`📦 ============================================`);
      console.log(`📦 Source: Service Item "${itemName}" (ID: ${id})`);
      console.log(`📦 Status: ${status}`);
      console.log(`📦 Workspace ID: ${workspace_id}`);
      console.log(`📦 --------------------------------------------`);
      ...
      try {
        ...
        console.log(`📦 Found ${connectionsResult.rows.length} connections where this service item is source`);

        if (connectionsResult.rows.length === 0) {
          console.log(`⚠️ No CMDB item connections found for service item ${id}`);
        }
        ...
        for (const conn of connectionsResult.rows) {
          console.log(`\n📦 Connection details:`, {
            ...
            type_name: conn.connection_type_name,
            propagation: conn.propagation,
          ...
          console.log(`\n📦 Processing connection to CMDB Item ${targetItemId}`);
          ...
          // For service-item-to-item, service item is source, CMDB item is target
          // propagation = 'source_to_target' means: if source (service item) is down, target (CMDB item) is affected
          // propagation = 'target_to_source' means: if target (CMDB item) is down, source (service item) is affected (not relevant here)
        ...
        console.log(`\n📦 --------------------------------------------`);
        console.log(`📦 CMDB ITEM PROPAGATION COMPLETED`);
        console.log(`📦 ============================================\n`);

        // Emit CMDB update to refresh frontend after propagation
        const { emitCmdbUpdate } = getSocketFunctions();
        await emitCmdbUpdate();
        console.log(`✅ Emitted cmdb_update after service item status propagation`);
      } catch (error) {
